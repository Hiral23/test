...
/*		$dataProvider=new CActiveDataProvider('FeesPaymentCheque');
	...
	public function actionCheque_result()
	{
		...
		{			
			...
			$fees_cheque=FeesPaymentCheque::model()->findByAttributes(array('fees_payment_cheque_number'=>$no,'fees_payment_cheque_organization_id'=>Yii::app()->user->getState('org_id')));

			if(isset($fees_cheque))		
				echo $this->renderPartial('search_result', array('model'=>$model,'fees_cheque'=>$fees_cheque));
			else
