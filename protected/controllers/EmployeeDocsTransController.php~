...
//		$docs = CUploadedFile::getInstancesByName('employee_docs_path');
			...
			if(isset($_POST['EmployeeDocs']))
			{
				...
				//var_dump($emp_doc->employee_docs_path);
				//exit;
				// if($emp_doc->employee_docs_path != NULL) {
					...
					$this->redirect(array('employeeTransaction/final_view','id'=>$model->employee_docs_trans_user_id));
					}
				/*else
				{
					$emp_doc->addError('employee_docs_path',"Please select any document1");
