		...
		if(isset($_POST['Organization']))
		{
			...
			$model->organization_creation_date=new CDbExpression('NOW()');
			 ...
			 $model->logo = CUploadedFile::getInstance($model,'logo');
					...
					$ext= substr(strrchr($model->logo,'.'),1);
					if($ext!=null)
					{						
						$newfname = $model->organization_name.'.'.$ext;
						$model->logo->saveAs(Yii::getPathOfAlias('webroot').'/organisation_logo/'.$model->logo=$newfname);
					}
					else
					{
						$newfname = $model->organization_name.'.jpg';
						$model->logo->saveAs(Yii::getPathOfAlias('webroot').'/organisation_logo/'.$model->logo=$newfname);
					}
			       ...
			       if($model->logo != null)
					...
					else
					{
						...
						$model->logo->saveAs(Yii::getPathOfAlias('webroot').'/organisation_logo/'.$model->logo=$newfname);
                               ...
                               if($model->save())
				$this->redirect(array('admin'));//$this->redirect(array('view','id'=>$model->organization_id));
		}
