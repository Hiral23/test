 ...
 * @property InwardDetail[] $inwardDetails
 */
class Inward extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('invoice_description, company_name, invoice_date,invoice_no_dc, received_date, sender_name, org_name_id,department, no_of_packets, receiver_name' , 'required','message'=>''),
			array('org_name_id, no_of_packets,department', 'numerical', 'integerOnly'=>true,'message'=>''),
			array('invoice_description', 'length', 'max'=>100),
			...
			array('document_desc', 'length', 'max'=>100),
			array('sender_name,receiver_name','CRegularExpressionValidator', 'pattern'=>'/^([A-Za-z  ]+)$/','message'=>''),

			//array('document', 'file', 'types'=>'txt, pdf'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, invoice_description, company_name, invoice_no_dc, invoice_date, received_date, sender_name, department, org_name_id, no_of_packets, receiver_name,department_name', 'safe', 'on'=>'search'),


			 array('photo, document', 'unsafe'),
		);
	}
