...
class AcademicTermPeriod extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			//array('academic_terms_period_name', 'unique','message'=>'Already Exist.'),
			array('academic_term_period','CRegularExpressionValidator','pattern'=>'/^([0-9-]+)$/','message'=>''),
			array('academic_term_period', 'checkacdmperiod'),
	...
	public function beforeSave()
	{
		...
		public static function items()
		{
	    	...
	    	{
			self::$_items=array();
			$models=self::model()->findAll(array('condition'=>'academic_terms_period_organization_id='.Yii::app()->user->getState('org_id')));
	...
	public function checkacdmperiod()
		{
			if($this->isNewRecord)
			{
				...
				if($acdm_period_name)
				{
					$this->addError('academic_term_period',"AcademicTermPeriod already exist.");	
					 return false;	
				}
				else
				{
