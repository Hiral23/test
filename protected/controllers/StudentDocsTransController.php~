...
//		$docs = CUploadedFile::getInstancesByName('student_docs_path');
			...
			if(isset($_POST['StudentDocs']))
			{
				...
				// if($stud_doc->student_docs_path != NULL) {
					...
					if($valid)
					{
					 $stud_doc->student_docs_path->saveAs(Yii::getPathOfAlias('webroot').'/stud_docs/' .$stud_doc->student_docs_path);
					 $stud_doc->save();
					...
					$this->redirect(array('studentTransaction/final_view','id'=>$model->student_docs_trans_user_id));
					}
				/*else
				{
